...
[`@ilbertt/apigen`](../../packages/apigen/pkg/README.md) with the **Bun.SQL** connector
on `Bun.serve`, over a small ecommerce schema. Differs from
[`with-postgres`](../with-postgres) only in `src/db.ts`.
